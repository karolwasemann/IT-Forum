...
export default function Topic3() {
    ...
    const getComments = async () => {
      ...
      const commentsApi = users.map((user) => {
        return {
          ...
          date: date,
          avatar:
            "https://gravatar.com/avatar/009892cea2b722e994c351366b70de61?s=400&d=robohash&r=x",
        };
      });
      let topic3Comments = JSON.parse(localStorage.getItem("topic3")) || [];
      if (topic3Comments.length === 0) {
        topic3Comments.push(
          ...commentsData.filter((com) => com.type === "topic3")
        );
        topic3Comments.push(...commentsApi);
        localStorage.setItem("topic3", JSON.stringify(topic3Comments));
      }
    };
    getComments();
    const getStored = () => {
      const storedComments = JSON.parse(localStorage.getItem("topic3"));
  ...
  const onDelete = (id) => {
    const _comments = JSON.parse(localStorage.getItem("topic3")).filter(
      (comment) => comment.id !== id
    );
    localStorage.setItem("topic3", JSON.stringify(_comments));
    setComments([..._comments]);
  };
  return (
    <div className="topic-container">
      <h2>Welcome at Topic CSS!!</h2>
